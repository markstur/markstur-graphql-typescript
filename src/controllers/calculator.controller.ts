  ...
  async operate(
    ...
    if (!operators.includes(operator)) {
      throw new Errors.NotFoundError(
        `There is no valid operator "${operator}"`
      );
    }
    ...
    const promiseArray = Promise.all(
      operandArray.map((s) => this.converter.toNumber(s))
    );
    ...
    if (calculated.length == 3) {
      const promiseConverted = Promise.all([
        this.converter.toRoman(calculated[0]),
        this.converter.toRoman(calculated[1]),
        this.converter.toRoman(calculated[2]),
      ]);
