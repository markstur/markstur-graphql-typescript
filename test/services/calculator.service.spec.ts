...
describe('Calculator service', () => {
  ...
  describe('When the calculator service', () => {
    ...
    context(
      'is given a sub operation with parameters "L, III, X, VI, I, IX"',
      () => {
        test('the expected result should be XXI : 50(L) - 3(III) - 10(X) - 6(VI) - 1(I) - 9(IX) = 21(XXI)', () => {
          expect(service.doMath('sub', [50, 3, 10, 6, 1, 9])).toStrictEqual([
            21,
          ]);
        });
      }
    ...
    context(
      'is given a mult operation with parameters "I, II, III, IV, V)',
      () => {
    ...
    context(
      'is given those crazy div with remainder scenarios -- Did the Romans do that???',
      () => {
